...
export default function MMSM06007E() {
  ...
  function onExportCsv() {
    ...
    const lines = rows.map((r) =>
      [r.DSP_SEQ ?? '', r.LINE_CD ?? '', r.LINE_GRP_CD ?? '', r.LINE_NM ?? '', r.USE_YN ?? '']
      ...
      <div className="flex flex-wrap items-end gap-3">
        <label className="flex flex-col text-sm">
          ...
          <input
            ...
            value={lineNm}
            onChange={(e) => setLineNm(e.target.value)}
          />
        </label>
              ...
              <tr key={i} className="border-b hover:bg-muted/30">
                ...
                <td className="p-1 text-center">
                  <input
                    ...
                    value={r.DSP_SEQ ?? ''}
                    onChange={(e) => patch(i, { DSP_SEQ: e.target.value })}
                  />
                </td>
                <td className="p-1 text-center">
                  <input
                    ...
                    value={r.LINE_CD ?? ''}
                    onChange={(e) => patch(i, { LINE_CD: e.target.value })}
                  />
                </td>
                <td className="p-1 text-center">
                  <input
                    className="h-8 border rounded px-2 w-full"
                    value={r.LINE_GRP_CD ?? ''}
                    onChange={(e) => patch(i, { LINE_GRP_CD: e.target.value })}
                  />
                </td>
                <td className="p-1">
                  <input
                    className={`h-8 border rounded px-2 w-full ${!r.LINE_NM ? 'border-destructive' : ''}`}
                    value={r.LINE_NM ?? ''}
                    onChange={(e) => patch(i, { LINE_NM: e.target.value })}
                  />
                </td>
                <td className="p-1 text-center">
                  <select
