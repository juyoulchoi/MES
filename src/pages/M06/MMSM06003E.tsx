...
export default function MMSM06003E() {
  ...
  async function fetchDetail(itemCd?: string) {
    ...
    return {
      cd1: data.cd1 ?? '',
      ITEM_TP1: data.ITEM_TP1 ?? '',
      PROC_CD1: data.PROC_CD1 ?? '',
      WID1: data.WID1 ?? '',
      HGT1: data.HGT1 ?? '',
      QTY1: data.QTY1 ?? '',
      cd2: data.cd2 ?? '',
      ITEM_TP2: data.ITEM_TP2 ?? '',
      PROC_CD2: data.PROC_CD2 ?? '',
      WID2: data.WID2 ?? '',
      HGT2: data.HGT2 ?? '',
      QTY2: data.QTY2 ?? '',
    } as Detail;
  ...
  async function onDelete() {
    ...
    if (targets.length > 0) {
      setLoading(true);
      try {
        await http(`/api/m06/mmsm06003/delete`, {
          method: 'POST',
          body: targets.map((cd) => ({ ITEM_CD: cd })),
        });
      } catch (e) {
        setError(e instanceof Error ? e.message : String(e));
      } finally {
  ...
  async function onSave() {
    ...
    try {
      const payload: AnyRow = {
        ...
        detail:
          selectedIndex >= 0 && master[selectedIndex]?.ITEM_CD
            ? {
  ...
  function onExportCsv() {
    ...
    const lines = master.map((r, i) =>
      [r.SERL ?? i + 1, r.ITEM_CD ?? '', r.ITEM_NM ?? '', r.PRT_CNT ?? '']
    ...
    a.href = url;
    a.download = 'MMSM06003E_master.csv';
      ...
      <div className="flex flex-wrap items-end gap-3">
        <label className="flex flex-col text-sm">
          ...
          <input
            ...
            value={itemNmFilter}
            onChange={(e) => setItemNmFilter(e.target.value)}
          />
        </label>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {master.map((r, i) => (
                <tr
                  key={i}
                  className={`border-b hover:bg-muted/30 ${selectedIndex === i ? 'bg-muted/30' : ''}`}
                ...
                >
                  ...
                  <td className="p-1 text-center" onClick={(e) => e.stopPropagation()}>
                    <input
                      ...
                      value={r.ITEM_CD ?? ''}
                      readOnly={!r.ISNEW}
                      onChange={(e) => patchRow(i, { ITEM_CD: e.target.value })}
                    />
                  </td>
                  <td className="p-1" onClick={(e) => e.stopPropagation()}>
                    <input
                      ...
                      value={r.ITEM_NM ?? ''}
                      onChange={(e) => patchRow(i, { ITEM_NM: e.target.value })}
                    />
                  </td>
                  <td className="p-1 text-center" onClick={(e) => e.stopPropagation()}>
                    <input
                      ...
                      value={r.PRT_CNT ?? ''}
                      onChange={(e) => patchRow(i, { PRT_CNT: e.target.value })}
                    />
                  </td>
                </tr>
              ))}
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 items-center">
                    ...
                    <div className="flex gap-1">
                      <input
                        className="h-8 border rounded px-2 w-full"
                        value={detail.ITEM_TP1 ?? ''}
                        readOnly
                      />
                      <button className="h-8 px-2 border rounded" onClick={() => openPicker('tp1')}>
                        ...
                      </button>
                    </div>
                    ...
                    <input
                      ...
                      value={detail.PROC_CD1 ?? ''}
                      onChange={(e) => patchDetail({ PROC_CD1: e.target.value })}
                    />
                    <div className="text-sm text-muted-foreground">가로</div>
                    <input
                      className="h-8 border rounded px-2"
                      value={detail.WID1 ?? ''}
                      onChange={(e) => patchDetail({ WID1: e.target.value })}
                    />
                    <div className="text-sm text-muted-foreground">세로</div>
                    <input
                      className="h-8 border rounded px-2"
                      value={detail.HGT1 ?? ''}
                      onChange={(e) => patchDetail({ HGT1: e.target.value })}
                    />
                    <div className="text-sm text-muted-foreground">절수</div>
                    <input
                      className="h-8 border rounded px-2"
                      value={detail.QTY1 ?? ''}
                      onChange={(e) => patchDetail({ QTY1: e.target.value })}
                    />
                  </div>
                </div>
                {/* Cardboard */}
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 items-center">
                    ...
                    <div className="flex gap-1">
                      <input
                        className="h-8 border rounded px-2 w-full"
                        value={detail.ITEM_TP2 ?? ''}
                        readOnly
                      />
                      <button className="h-8 px-2 border rounded" onClick={() => openPicker('tp2')}>
                        ...
                      </button>
                    </div>
                    <div className="text-sm text-muted-foreground">투입위치</div>
                    <input
                      className="h-8 border rounded px-2"
                      value={detail.PROC_CD2 ?? ''}
                      onChange={(e) => patchDetail({ PROC_CD2: e.target.value })}
                    />
                    <div className="text-sm text-muted-foreground">가로</div>
                    <input
                      className="h-8 border rounded px-2"
                      value={detail.WID2 ?? ''}
                      onChange={(e) => patchDetail({ WID2: e.target.value })}
                    />
                    <div className="text-sm text-muted-foreground">세로</div>
                    <input
                      className="h-8 border rounded px-2"
                      value={detail.HGT2 ?? ''}
                      onChange={(e) => patchDetail({ HGT2: e.target.value })}
                    />
                    <div className="text-sm text-muted-foreground">절수</div>
                    <input
                      className="h-8 border rounded px-2"
                      value={detail.QTY2 ?? ''}
                      onChange={(e) => patchDetail({ QTY2: e.target.value })}
                    />
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-background border rounded p-3 w-[360px] space-y-2 shadow-lg">
            <div className="font-semibold">
              {pickerOpen === 'tp1' ? '종이 종류 선택' : '판지 종류 선택'}
            </div>
            <input
              className="h-8 border rounded px-2 w-full"
              value={pickerVal}
            ...
            <div className="flex justify-end gap-2 pt-1">
              <button className="h-8 px-3 border rounded" onClick={() => setPickerOpen(null)}>
                취소
              </button>
              <button
