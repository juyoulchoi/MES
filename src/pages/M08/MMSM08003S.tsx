...
export default function MMSM08003S() {
  ...
  async function onSearch() {
    ...
    try {
      ...
      const url = `/api/m08/mmsm08003/list` + (params.toString() ? `?${params.toString()}` : '');
  ...
  function emitSelection(r?: Row) {
    ...
    try { window.opener && window.opener.postMessage({ type: 'MMSM08003S_SELECT', payload }, '*'); } catch {}
    try { window.parent && window.parent !== window && window.parent.postMessage({ type: 'MMSM08003S_SELECT', payload }, '*'); } catch {}
  }
  ...
  function onExportCsv() {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'MMSM08003S_customers.csv';
    ...
    <div className="p-3 space-y-3" style={{ width: 640 }}>
      ...
      <div className="flex flex-wrap items-end gap-3">
        <label className="flex flex-col text-sm">
          <span className="mb-1">거래처코드</span>
          <input className="h-8 border rounded px-2 w-48" value={cstCd} onChange={e=>setCstCd(e.target.value)} />
        </label>
        <label className="flex flex-col text-sm">
          <span className="mb-1">거래처명</span>
          <input className="h-8 border rounded px-2 w-48" value={cstNm} onChange={e=>setCstNm(e.target.value)} />
        </label>
        ...
        <table className="w-full text-sm">
          <thead className="sticky top-0 bg-background">
            <tr className="border-b">
              ...
              <th className="w-40 p-2 text-center">고객사ID</th>
              <th className="w-56 p-2 text-left">고객사명</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => (
