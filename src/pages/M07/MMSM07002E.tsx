          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className="border-b hover:bg-muted/30">
                ...
                <td className="p-1">
                  <input
                    className={`h-8 border rounded px-2 w-full ${!r.PGM_ID ? 'border-destructive' : ''}`}
                    value={r.PGM_ID ?? ''}
                    onChange={(e) => patch(i, { PGM_ID: e.target.value })}
                  />
                </td>
                <td className="p-1">
                  <input
                    className={`h-8 border rounded px-2 w-full ${!r.PGM_NM ? 'border-destructive' : ''}`}
                    value={r.PGM_NM ?? ''}
                    onChange={(e) => patch(i, { PGM_NM: e.target.value })}
                  />
                </td>
                <td className="p-1">
                  <input
                    ...
                    value={r.PGM_URL ?? ''}
                    onChange={(e) => patch(i, { PGM_URL: e.target.value })}
                    readOnly
                  />
                </td>
